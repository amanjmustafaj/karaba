...
class ElectricityCalculator:
    ...
    def run(self):
        ...
        if st.session_state.mode == "kwh_to_dinar":
            ...
            if st.button("هەژمارکردن", type="primary", use_container_width=True, key="calc1"):
                if kwh > 0:
            ...
            if st.button("هەژمارکردن", type="primary", use_container_width=True, key="calc2"):
                if money > 0:
        ...
        if category == "ماڵان":
            ...
            for idx, (limit, price) in enumerate(self.home_tiers):
                if temp_usage > 0:
                    ...
                    with col1: st.write(f"**{tier_names[idx]}**")
                    with col2: st.write(f"{consumed:,.0f} kWh")
                    with col3: st.write(f"{price} دینار")
                    with col4: st.write(f"{cost:,.0f} دینار")
                    temp_usage -= consumed
            st.markdown("---")
